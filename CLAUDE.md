...
23. Local Executor Bridge (`local_executor.py`) — The frontend spawns a local Python subprocess to execute actions when Nova Act is enabled. Real-time status updates are emitted via `stdout` JSON lines and displayed in the React UI, while the remote AppSync subscriptions are ignored to prevent duplicate action cards.
